...
export default function AddProductPanel() {
  ...
  const handleChange = (e, newValue, id) => {
    if (
      ...
      (Boolean(id) || Boolean(productId) || Boolean(productIdEdit))
    ) {
  ...
  return (
    <DashboardLayout>
      ...
      <Box sx={{ mx: 4, my: 4 }}>
        ...
        >
          <Tab
            label="Product details  (1st step)"
            {...a11yProps(0)}
            sx={{ color: value == 0 && "#5D449B !important" }}
          />
          <Tab
            label="Product images (2nd step)"
            {...a11yProps(1)}
            sx={{ color: value == 1 && "#5D449B !important" }}
          />
          <Tab
            label="Attributes (3rd step)"
            {...a11yProps(2)}
            sx={{ color: value == 2 && "#5D449B !important" }}
          />
        </Tabs>
      </Box>
