...
const ProductAttributes = ({ idProduct, handleChange }) => {
  ...
  let [counter, setCounter] = React.useState(
    controls.values.length > 0 ? controls.values.length - 1 : controls.values.length
  );
  const handleCloseDialog = () => {
    setOpenDialog(false);
  };
  const [attributepostRequest, postattributeResponce] = useRequest({
    path: ATTRIBUTES,
    method: "post",
    Token: `Token ${Token}`,
    successMessage: t("addnewattributemessage"),
  });
  const [attributeEditRequest, editattributeResponce] = useRequest({
    path: ATTRIBUTES,
    method: "patch",
    Token: `Token ${Token}`,
  ...
  const [attributeValueeditRequest, editValueattributeResponce] = useRequest({
    path: ATTRIBUTES,
    method: "patch",
    Token: `Token ${Token}`,
    // contentType: "multipart/form-data",
  });
  const [productvariantRequest, productvariantResponce] = useRequest({
    path: PRODUCTS,
    method: "get",
    Token: `Token ${Token}`,
    // contentType: "multipart/form-data",
  });
  const [productvariantupdate, productvariantupdateResponce] = useRequest({
    path: PRODUCTS,
    method: "patch",
    Token: `Token ${Token}`,
    // contentType: "multipart/form-data",
        ...
        attributeEditRequest({
          ...
          onSuccess: (res) => {
            ...
            attributeValueeditRequest({
              ...
              onSuccess: (res) => {
                dispatch({
                  type: "attribute/addValues",
                  payload: { idattribute: controls.id, values: res.data },
                });
                setControl(
                  "attribute",
                  controls.attribute.map((elem) =>
                    elem.id == controls.id
                      ? {
                          ...elem,
                          values: controls.values.map((ele) =>
                            Boolean(ele.id)
                              ? ele
                              : res.data.find((elem) => elem.value_name == ele.value_name)
                          ),
                        }
                      : elem
                  )
  ...
  function AddnewValue() {
    if (controls.iscolor) {
      if (Boolean(controls.value_name) && Boolean(controls.color_value)) {
        ...
        let match = test.test(controls.color_value);
        if (match) {
          setControl("attributenewvalues", [
            ...controls.attributenewvalues,
            {
              value_name: controls.value_name,
              color_value: controls.color_value,
              iscolor: controls.iscolor,
  ...
  function onDeleteNew(row) {
    setControl(
      "values",
      controls.values.filter((ele, index) => index !== row)
    );
    setEdit(null);
  }
  const [attributeRequest, attributeResponse] = useRequest({
    path: ATTRIBUTES,
    method: "get",
    Token: `Token ${Token}`,
  });
  ...
  const [editattributeValueRequest, ValueattributeeditResponce] = useRequest({
    path: ATTRIBUTES,
    method: "patch",
    Token: `Token ${Token}`,
    // contentType: "multipart/form-data",
  });
  const [GenerationAttributesRequest, GenerationAttributeResponse] = useRequest({
    path: "shop/dashboard/generate/attributes/",
    method: "POST",
    Token: `Token ${Token}`,
  });
  const [deleteVariantRequest, deleteVariantResponse] = useRequest({
    path: PRODUCTS,
    method: "delete",
    Token: `Token ${Token}`,
  });

  const [variantAttributesRequest, variantAttributeResponse] = useRequest({
    path: PRODUCTS,
    method: "POST",
    Token: `Token ${Token}`,
  });
  const [attributeDeleteRequest, DeleteattributerResponce] = useRequest({
    path: ATTRIBUTES,
    method: "delete",
    Token: `Token ${Token}`,
  ...
  const handleClosepopup = () => {
    setControl(
      "attribute",
      controls?.attribute?.map((ele) =>
        ele.id == popupvalue.id
          ? { ...ele, values: controls.attributevalues.map((elem) => elem) }
          : ele
      )
    ).then(() => {
  ...
  const postGenerationAttribute = () => {
    ...
    GenerationAttributesRequest({
      body: {
        attributes: controls?.attribute.map((ele) => ({
          attribute: ele.name,
          values: ele.values.map((elem) => elem.value_name),
        })),
      },
      onSuccess: (res) => {
  ...
  const postGenerationAttributes = () => {
    if (
      controls.variants.filter(
        (ele) =>
          (Boolean(ele.title) ||
            Boolean(ele.sku) ||
            Boolean(ele.quantity) ||
            Boolean(ele.price) ||
            Boolean(ele.gtin)) &&
          !Boolean(ele.id)
      ).length > 0
      ...
      variantAttributesRequest({
        ...
        onSuccess: (res) => {
          if (res?.data[0]) {
            dispatch({
              type: "products/addVariantProperty",
              payload: { id: idproduct, item: res?.data[0] },
            });
            setInvalid({ variants: [] });
        ...
        if (response?.length > 0) {
          setInvalid({
            variants: [...controls.variants.filter((ele) => ele.id).map((elem) => {}), ...response],
          });
        }
      });
    }
    if (
      ...
      let newvariant = controls.variants.filter(
        (ele) =>
          (Boolean(ele.title) ||
            Boolean(ele.sku) ||
            Boolean(ele.quantity) ||
      ...
      let result =
        newvariant.length > 0 &&
        newvariant?.map((ele, index) =>
          compare(
            [
              [String(ele?.quantity), String(variant[index]?.quantity), "quantity"],
      ...
      productvariantupdate({
        ...
        body: result
          .map((ele, index) => ele.nochange && { variant_id: newvariant[index].id, ...ele.array })
          ?.filter((ele) => Boolean(ele)),
        onSuccess: (res) => {
          res.data.map((elem) =>
            dispatch({
              type: "products/updateVariantProperty",
              payload: { id: idproduct, idvariant: elem.id, item: elem },
            })
          );
  ...
  function deleteVariant(ele) {
    if (ele.id) {
      deleteVariantRequest({
        ...
        onSuccess: (res) => {
          dispatch({
            type: "products/deleteVariantProperty",
            payload: { id: idproduct, idVariant: ele.id },
          });
        },
      });
    } else {
      setControl(
        "variants",
        controls.variants.filter((elem) => elem != ele)
      );
    }
  }
  function onDeleteValue(row, valueId) {
    attributeDeleteRequest({
      ...
      onSuccess: () => {
        dispatch({
          type: "attribute/deleteValueofAttribute",
          payload: { idattribute: row, idValue: valueId },
        });
        setControl(
          "values",
          controls.values.filter((ele) => ele.id !== valueId)
        );
      },
    });
  }
  ...
  function editValue(ele) {
    ...
    editattributeValueRequest({
      ...
      onSuccess: (res) => {
        dispatch({
          type: "attribute/editValue",
          payload: { id: controls.id, idvalue: ele.id, item: res.data },
        });
        setControl(
          "values",
          controls.values.map((elem) => (elem.id == ele.id ? res.data : elem))
        );
  ...
  function editNew(element, ind) {
    {
      openDialogEdit
        ? setControl(
            "values",
            controls.values.map((element, index) =>
              index == ind
                ? {
                    ...element,
                    color_value: Boolean(controls.color_value)
                      ? controls.color_value
                      : element.color_value,
                    value_name: Boolean(controls.value_name)
                      ? controls.value_name
                      : element.value_name,
                  }
                : element
        ...
        : setControl(
            "attributenewvalues",
            controls.attributenewvalues.map((element, index) =>
              index == ind
                ? {
                    ...element,
                    color_value: Boolean(controls.color_value)
  ...
  useEffect(() => {
    if (controls.attribute.length > 0) {
      ...
      setControl(
        "values",
        controls.attribute[controls.attribute.length - 1].values.map((ele) => ele)
      );
    } else {
      setInvalid("variants", []);
      setpopupvalue(null);
  ...
  return (
    <Container maxWidth="xl">
      ...
      >
        ...
        <Box mt={"24px"}>
          <SelectField
            ...
            renderValue={(selected) => {
              return attributes?.find((attribute) => attribute?.id === selected)?.name;
            }}
            isPending={attributeResponse.isPending}
            value={controls?.attribute}
            onChange={(e) => {
              setControl("attribute", [...controls?.attribute, e.target.value]);
            }}
          ...
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            ...
            <TableBody>
              {controls.attribute.map((row) => (
                ...
                >
                  <TableCell align="start" sx={{ border: "1px solid ", borderColor: "#d9d9d9" }}>
                    {row.name}
                  </TableCell>
                  <TableCell align="start" sx={{ border: "1px solid ", borderColor: "#d9d9d9" }}>
                    {row?.values?.length > 0 ? row?.values?.length : 0} values
                  </TableCell>
                  <TableCell
                    align="center"
                    sx={{ border: "1px solid ", borderColor: "#d9d9d9" }}
                    gap={1}
                  >
                    <img
                      src={Edit}
        ...
        {generate || controls?.variants?.length > 0 ? (
          <Box>
            <Typography sx={{ color: (theme) => theme.palette.grey[600], marginY: "20px" }}>
              {t("note")}
            </Typography>
            <TableContainer component={Paper}>
              <Table
                sx={{ minWidth: 650, color: (theme) => theme.palette.grey[300] }}
                aria-label="caption table"
              >
                <TableBody>
                  {/* {rows.map((row) => ( */}
                  {controls?.variants?.map((ele, index) => (
                    <TableRow
                      key={index}
                      sx={{
                        display: "flex",
                    ...
                    >
                      ...
                      <TableCell align="right" sx={{ width: "70%", height: "70px" }}>{`${
                        Boolean(ele?.attributes)
                          ? ele?.attributes?.join("")
                          : ele?.variant_attributes?.map((elem) => elem?.value_name)?.join("")
                      }`}</TableCell>
                      <TableCell align="right">
                        {" "}
                        <SoftInput
                          ...
                          onChange={(e) =>
                            setControl(
                              "variants",
                              controls.variants.map((elem, ind) =>
                                ind == index
                                  ? {
                                      ...ele,
                          ...
                          error={Boolean(
                            invalid?.variants && invalid?.variants[index]
                              ? invalid?.variants[index]?.quantity
                              : ""
                          )}
                          helperText={
                            invalid?.variants && invalid?.variants[index]
                      ...
                      <TableCell align="right" sx={{ color: (theme) => theme.palette.error.main }}>
                        <DeleteIcon onClick={() => deleteVariant(ele)} />
                      </TableCell>
                    </TableRow>
                  ))}
      ...
      <Box
        sx={{ display: "flex", justifyContent: "flex-end", alignItems: "center", marginY: "14px" }}
      >
        <SoftButton
          variant="contained"
          color="white"
        ...
        <SoftButton
          ...
          disabled={
            controls?.variants?.filter(
              ...
              ? variantAttributeResponse.isPending
              : productvariantupdateResponce.isPending
          }
          sx={{
      ...
      >
        ...
        >
          ...
          >
            {t("descributionAttribute")}
          </DialogTitle>
          <Typography
            variant={"label"}
            sx={{ display: "block", fontSize: "14px", marginBottom: "5px" }}
          >
            {t("Attributename")}
          </Typography>
          <SoftInput
          ...
          <SoftBox sx={{ display: "flex" }}>
            <Checkbox
              checked={controls.iscolor}
              onClick={() => setControl("iscolor", !controls.iscolor)}
            />
          ...
          <SoftBox sx={{ border: "1px solid #8080807d", borderRadius: "8px" }}>
            ...
            {openDialogEdit
              ? controls?.values?.map((ele, index) => (
                  ...
                  >
                    ...
                    {Boolean(edit) && edit?.id == ele?.id && index == indexedit ? (
                      ...
                      <TableCell sx={{ width: "14rem", borderRight: "1px solid #8080807d" }}>
                        {ele.value_name}
                      </TableCell>
                    )}
                      ...
                      Boolean(edit) && edit?.id == ele?.id && index == indexedit ? (
                        <TableCell
                          sx={{
                            ...
                            alignItems: "center",
                            borderRight: "1px solid #8080807d",
                          }}
                        >
                          <SoftInput
                            placeholder="color"
                          ...
                          <Typography sx={{ fontSize: "14px" }}>
                            {controls.color_value ? controls.color_value : edit?.color_value}
                          </Typography>
                        </TableCell>
                      ) : (
                        <TableCell sx={{ width: "14rem", borderRight: "1px solid #8080807d" }}>
                    ...
                    {attributes
                      ?.find((ele) => ele.id == controls.id)
                      ?.values.map((ele) => ele.value_name)
                      .includes(ele.value_name) ? (
                      <TableCell
                        sx={{
                          width: "50%",
                      ...
                      >
                        {Boolean(edit) && edit?.id == ele?.id && index == indexedit ? (
                          <SoftBox>
                            <SaveAsIcon onClick={() => editValue(ele)} />
                          </SoftBox>
                        ) : (
                          <EditIcon
                            onClick={() => {
                        ...
                        {Boolean(edit) && edit?.id == ele?.id && index == indexedit ? (
                          <SoftBox>
                            {" "}
                            <CloseIcon onClick={() => setEdit(null)} />
                          </SoftBox>
                        ) : (
                          <DeleteIcon
                            sx={{ color: (theme) => theme.palette.error.main, cursor: "pointer" }}
                            onClick={() => onDeleteValue(controls.id, ele.id)}
                          />
                        )}
                      </TableCell>
                    ) : (
                      ...
                      >
                        {Boolean(edit) && edit?.id == ele?.id && index == indexedit ? (
                          <SaveAsIcon onClick={() => editNew(ele, index)} />
                        ) : (
                          <EditIcon
                            onClick={() => {
                              setEdit(ele);
              ...
              : controls?.attributenewvalues?.map((ele, index) => (
                  <TableRow
                    key={ele.id}
                    sx={{
                      fontSize: "14px",
            ...
            {addvalue && !controls.iscolor ? (
              <TableRow>
                ...
                >
                  <SoftInput
                    ...
                    onChange={(e) => {
                      setControl("value_name", e.target.value);
                      setblurvalue(true);
                    }}
                    required={required.includes("value_name")}
                    error={Boolean(invalid?.value_name)}
              ...
              addvalue &&
              controls.iscolor && (
                <TableRow onChange={() => setblurvalue(true)}>
                  ...
                  <TableCell
                    sx={{
                      borderRight: "1px solid #80808059",
                      width: "10.3rem",
                      display: "flex",
                      justifyContent: "space-between",
                    }}
      ...
      >
        <Container sx={{ my: "20px" }}>
          <SelectField
            ...
            // onOpen={getAttributies}
            renderValue={(selected) => {
              let resultcategory = controls?.values?.filter((category) =>
                selected.map((ele) => ele.id).includes(category.id)
              );
            ...
            onChange={(e) => {
              setControl("attributevalues", e.target.value);
            }}
