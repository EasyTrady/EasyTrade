...
function Category({ absolute, light, isMini }) {
  ...
  const handleAddNewCategory = () => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      navigate(`/${sub_domain}/dashboard/products/category/addnewCategory`);
    }, 1000);
  };
      ...
      renderCell: (params) => {
        ...
        return (
          <Stack direction={"row"} alignItems={"center"}>
            <SoftBox sx={{ width: "64px", height: "64px", borderRadius: "8px" }}>
              <img src={row.image} style={{ width: "100%", borderRadius: "8px" }} />
            </SoftBox>
            <Typography component={"p"} sx={{ fontSize: "14px", marginX: "10px" }}>
              {row.name}
            </Typography>
        ...
        return (
          <Stack direction={"row"}>
            {!row.is_root_node && (
              <Stack direction={"row"} alignItems={"center"}>
                <SoftBox sx={{ width: "64px", height: "64px", borderRadius: "8px" }}>
                  <img src={row.parent.image} style={{ width: "100%", borderRadius: "8px" }} />
                </SoftBox>
                <Typography component={"p"} sx={{ fontSize: "14px", marginX: "10px" }}>
                  {row.parent.name}
                </Typography>
  ...
  const [categorydeleteRequest, deletecategoryResponce] = useRequest({
    path: CATEGORY,
    method: "delete",
    Token: `Token ${Token}`,
  });
    ...
    <DashboardLayout>
      ...
      <Container sx={{ p: 2 }}>
        ...
        >
          ...
          {permissionYour.map((ele) => ele.codename).includes("add_category") && (
            <SoftButton
            variant="gradient"
            sx={{
