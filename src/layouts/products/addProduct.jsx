...
const AddProduct = ({ light, isMini, handleChange }) => {
  ...
  const addKey = () => {
    ...
    if (Boolean(controls.key) && Boolean(controls.value)) {
      setControl("specifications", [
        ...controls.specifications,
        { key: controls.key, value: controls.value },
      ]);
      setControl("key", "");
      setControl("value", "");
    }
        ...
        let result = compare(
          [
            ...
            [
              ...
              product?.is_percentage_discount,
              "is_percentage_discount",
            ],
            [controls?.discount, product?.discount, "discount"],
            [
              new Date(controls?.discount_start_date)?.toISOString(),
              product?.discount_start_date,
              "discount_start_date",
            ],
            [
              new Date(controls?.discount_end_date)?.toISOString(),
              product?.discount_end_date,
              "discount_end_date",
            ],
            [controls?.require_shipping, product?.require_shipping, "require_shipping"],
            [
              controls?.maximum_order_quanitity,
              product?.maximum_order_quanitity,
              "maximum_order_quanitity",
            ],
        ...
        AddProductRequest({
          body: filter({
            obj: {
              ...
              discount_start_date:
                controls?.discount_start_date && controls?.discount_start_date?.toISOString(),
              discount_end_date:
                controls?.discount_start_date && controls?.discount_end_date?.toISOString(),
    ...
    if (ele.id) {
      specificationsdeleteRequest({
        ...
        onSuccess: (res) => {
          setControl(
            "specifications",
            controls?.specifications.filter((elem, ind) => elem != ele)
          );
        },
      });
    } else {
      ...
      getProductRequest({
        ...
        onSuccess: (res) => {
          ...
          Object.entries(res.data)?.forEach(([key, value]) =>
            Object.keys(controls).includes(key) ? setControl(key, value) : null
          );
        },
      });
    }
    ...
    <>
          ...
          >
            ...
            <MultiSelect
              ...
              renderValue={(selected) => {
                ...
                let resultcategory = category?.results?.filter((category) =>
              ...
              onChange={(e) => {
                setControl("categories", e.target.value);
              }}
            ...
            <MultiSelect
              ...
              onChange={(e) => {
                setControl("brand", e.target.value);
              }}
              ...
              SelectProps={{
                ...
                // onClose: onClose,
                renderValue: (selected) => {
                  let resultcategory = Brands?.results?.filter(
      ...
      <Box sx={{ background: "#FFFFFF", borderRadius: "8px", height: "100%", mt: 2.5 }}>
        <AddProductTitle
          ...
          switch={controls.switchSpecifications}
          func={() => setControl("switchSpecifications", !controls.switchSpecifications)}
        />
        {controls.switchSpecifications && (
          <TableContainer component={Paper} sx={{ boxSizing: "border-box" }}>
            <Table sx={{ margin: "20px 24px 24px 24px", width: "94%" }} aria-label="simple table">
              ...
              <TableBody>
                {controls?.specifications &&
                  controls?.specifications?.map((ele, index) => (
                    <TableRow key={index}>
                      <TableCell>{ele?.key}</TableCell>
                      <TableCell>{ele?.value}</TableCell>
                      <TableCell>
                        <DeleteIcon onClick={() => DeleteSpecification(ele)} />
                      </TableCell>
                    </TableRow>
                  ))}
                {key && (
                  <TableRow>
                    <TableCell align="right">
                      <SoftInput
                        placeholder="key"
                        sx={{
                          ".MuiInputBase-root": { border: `unset`, padding: "0px !important" },
                        }}
                        value={controls.key}
                    ...
                    <TableCell align="right">
                      <SoftInput
                        ...
                        onChange={(e) => setControl("value", e.target.value)}
        ...
        <Container sx={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          >
            <SelectValueWeight
              ...
              placeholder={"Weight"}
              type={
                typeof controls.weight === "object"
                  ? String(controls.weight.unit)
                  : controls.weight_unit
              }
              value={
                typeof controls.weight === "object"
                  ? String(controls.weight.value)
                  : String(controls.weight)
              }
          ...
          <Container sx={{ display: "flex", flexDirection: "column", gap: "20px", mt: "20px" }}>
            ...
            <Box>
              ...
              <DatePickerField
                ...
                onChange={(newvalue) => {
                  setControl("discount_start_date", newvalue);
                }}
                icon={DateIcon}
              />
            </Box>
      ...
      <Box sx={{ display: "flex", justifyContent: "flex-end", alignItems: "center", mt: "24px" }}>
        ...
        <SoftButton
          ...
          variant="gradient"
          disabled={
            Boolean(productId) ? patchProductResponce.isPending : AddProductResponce.isPending
          }
          sx={{
        ...
        >
          {Boolean(productId) ? (
            patchProductResponce.isPending ? (
              <>
                <CircularProgress />
                loading
              </>
