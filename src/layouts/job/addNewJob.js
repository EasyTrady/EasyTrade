...
function AddNewJob({ absolute, light, isMini }) {
  ...
  const [JobPostRequest, PostJobrResponce] = useRequest({
    path: JOBS,
    method: "post",
    Token: `Token ${Token}`,
  });
  const [jobpatchRequest, patchjobrResponce] = useRequest({
    path: JOBS,
    method: "patch",
    Token: `Token ${Token}`,
  });
      ...
      <DashboardLayout>
        ...
        <Container>
          ...
          >
            <Breadcrumbs
              icon="home"
              title={Boolean(state?.dataRow) ? t("Edit Job") : "Add New Job"}
              route={route}
              light={light}
            />
          </SoftBox>
                      ...
                      index <= array.length / 2 && (
                        ...
                        >
                          ...
                          <SoftBox sx={{ display: "flex", alignItems: "center" }}>
                            ...
                            <Switch
                              checked={controls?.job_permissions?.includes(ele?.codename)}
                              color="default"
                              onChange={() =>
                                setControl(
                                  "job_permissions",
                                  controls?.job_permissions?.includes(ele?.codename)
                                    ? controls?.job_permissions.filter(
                                        (elem) => elem != ele?.codename
                                      )
                                    : [...controls?.job_permissions, ele?.codename]
                                )
                              }
                            />
                          </SoftBox>
