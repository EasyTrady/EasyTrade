...
function Job({ absolute, light, isMini }) {
  ...
  const [jobpostRequest, postjobResponce] = useRequest({
    path: JOBS,
    method: "post",
    Token: `Token ${Token}`,
  });
  ...
  const [jobDeleteRequest, DeletejobrResponce] = useRequest({
    path: JOBS,
    method: "delete",
    Token: `Token ${Token}`,
  });
  ...
  const handleAddNewJop = () => {
    setIsLoading(true);

    setTimeout(() => {
      setIsLoading(false);
      navigate(`/${sub_domain}/dashboard/jobs/addNewJob`);
    }, 1000);
  };
  const columns = [
    {
    ...
    <DashboardLayout>
      ...
      <Container sx={{ p: 2 }}>
        ...
        >
          ...
          {permissionYour.map((ele) => ele.codename).includes("add_employeejob") && (
            <SoftButton
            variant="gradient"
            sx={{
