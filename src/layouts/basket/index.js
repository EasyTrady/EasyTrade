...
function Basket({ absolute, light, isMini }) {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box>
          <Typography
            variant={"h6"}
            sx={{ color: (theme) => theme.palette.black.main, marginTop: "10px" }}
          >
            {params.row.customer_name}
          </Typography>
          <Typography
            variant={"p"}
            sx={{ color: (theme) => theme.palette.grey[500], marginTop: "10px" }}
          >
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box sx={{ display: "flex", flexDirection: "column" }}>
          ...
          >
            {params.row.is_notified_before ? (
              <Typography variant={"p"}>Notified</Typography>
            ) : (
              <Typography variant={"p"}>Not notified</Typography>
            )}
          </Typography>
        </Box>
      ),
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box sx={{ display: "flex", flexDirection: "column" }}>
          <Typography
            variant={"h6"}
            sx={{ color: (theme) => theme.palette.primary, marginTop: "10px" }}
          >
            {params.row.total}EGP
          </Typography>
        </Box>
      ),
      filterable: false,
  ...
  const [cartRequest, getCartResponce] = useRequest({
    path: CARTLEFT,
    method: "get",
    Token: `Token ${Token}`,
  });
  const [productRequest, getproductResponce] = useRequest({
    path: PRODUCTS,
    method: "get",
    Token: `Token ${Token}`,
  });
  const [SendEmailProductRequest, getsendemailproductResponce] = useRequest({
        ...
        >
          ...
          >
            ...
            <DialogContent sx={{ padding: 0 }}>
              <Typography
                variant={"label"}
                sx={{ display: "block", fontSize: "14px", marginBottom: "10px" }}
              >
                {t("emailTitle")}
              </Typography>
              <SoftInput
              ...
              <FormLabel id="demo-row-radio-buttons-group-label" sx={{ fontSize: "14px" }}>
                {t("contentType")}
              </FormLabel>
              <RadioGroup
                row
                ...
                sx={{ marginX: "20px" }}
                onChange={(e) => {
                  setControl("content_type", e.target.value);
                }}
              >
                <FormControlLabel
                  value={t("Banner")}
                  control={<Radio />}
                  label={t("Banner")}
                  sx={{ ".MuiFormControlLabel-label": { fontWeight: "400", fontSize: "12px" } }}
                />
                <FormControlLabel
                  value={t("Multipleproducts")}
                  control={<Radio />}
                  label={t("Multipleproducts")}
                  sx={{ ".MuiFormControlLabel-label": { fontWeight: "400", fontSize: "12px" } }}
                />
              </RadioGroup>
              ...
              ) : controls.content_type === "Multiple products" ? (
                <>
                  <FormLabel id="demo-row-radio-buttons-group-label" sx={{ fontSize: "14px" }}>
                    {t("SelectProducts")}
                  </FormLabel>
                ...
                /> */}
                  <SoftInput
                    ...
                    onChange={(e) => {
                      setControl(
                        "Products",
                        [...controls?.Products?.map((elem) => elem.id), e.target.value].map((ele) =>
                          products?.results.find((elem) => elem.id === ele)
                        )
                      );
                      validate();
                    }}
                    ...
                    SelectProps={{
                      ...
                      renderValue: (selected) => {
                        if (!Boolean(selected)) {
                          return (
                            <Typography
                              sx={{ opacity: "0.42", fontSize: "14px", color: "#959FA3" }}
                              variant="p"
                            >
                              {"Vendor"}
                            </Typography>
                  ...
                  {controls.Products.map((ele, index) =>
                    index <= 2 ? (
                      <SoftBox
                        key={ele}
                        sx={{
                      ...
                      >
                        <SoftBox sx={{ display: "flex", alignItem: "center" }}>
                          ...
                          <Typography
                            sx={{ color: "gray", fontSize: "14px !important", marginX: "10px" }}
                            variant={"span"}
                          >
                            {ele?.name}
                          </Typography>
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setControl(
                              "Products",
                              controls.Products.filter((elem) => elem?.id != ele?.id)
                            )
                          }
                        >
                          {" "}
