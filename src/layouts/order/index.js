...
function Order({ absolute, light, isMini }) {
  ...
  const columns = [
      ...
      renderCell: (params) => {
        ...
        return (
          <Typography
            ...
            onClick={() => navigate(`/${sub_domain}/dashboard/order/${row.id}`)}
          >
            {row.id}
          </Typography>
        );
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Typography sx={{ color: "#1B53C5", fontSize: "14px" }}>
    ...
    },
    {
      ...
      disableColumnMenu: true,
      renderCell: (params) =>
        params.row.status_name === "In Progress" ? (
          <Typography
            sx={{
        ...
        ) : params.row.status_name === "Cancelled" ? (
          <Typography
            sx={{
        ...
        ) : params.row.status_name === "Delivered" ? (
          <Typography
            sx={{
        ...
        ) : params.row.status_name === "Shipped" ? (
          <Typography
            sx={{
  ...
  const [OrderRequest, getOrderResponce] = useRequest({
    path: ORDERS,
    method: "get",
    Token: `Token ${Token}`,
  });
  const [OrderDeleteRequest, deleteOrderResponce] = useRequest({
    path: ORDERS,
    method: "delete",
    Token: `Token ${Token}`,
  });
    ...
    <DashboardLayout>
      ...
      <Container maxWidth={false} sx={{ p: 4 }}>
        ...
        <DataGridCustom
          ...
          onRowClick={(e) =>
            permissionYour.map((ele) => ele.codename).includes("view_orderitem") &&
            navigate(`/${sub_domain}/dashboard/order/${e?.row.id}`)
          }
