...
function AddVendor({ absolute, light, isMini }) {
  ...
  const [venderRequest, venderResponce] = useRequest({
    path: VENDER,
    method: "post",
    Token: `Token ${Token}`,
    contentType: "multipart/form-data",
  });
  const [vendorPermissionRequest, vendorpermissionResponce] = useRequest({
    path: VENDORPERMISSION,
    method: "get",
    Token: `Token ${Token}`,
  });
  const [vendorupdateRequest, vendorupdateResponce] = useRequest({
    path: VENDER,
    method: "patch",
    Token: `Token ${Token}`,
    contentType: "multipart/form-data",
  ...
  useEffect(() => {
    ...
    if (Boolean(state?.dataRow)) {
      Object.entries(state?.dataRow)?.forEach(([key, value]) => {
        key == "logo" ? setControl("logo", value) : setControl(key, value);
      });
  ...
  function handleSubmit() {
    validate().then((output) => {
      ...
      if (Boolean(state?.dataRow)) {
        ...
        Object.entries(result.array).map(([key, value]) =>
          key != "permissions" ? newForm.append(key, value) : newForm.append("permissions[]", value)
        );
        vendorupdateRequest({
          id: controls.id,
          body: newForm,
        ...
        Object.entries(controls).map(([key, value]) =>
          key != "image" && key != "permissions" && key != "logo" && key != "phone"
            ? newForm.append(key, String(value))
            : key != "image" && key != "phone" && key != "permissions" && key != "logo"
  ...
  return (
    <DashboardLayout>
      ...
      <Container maxWidth={false} sx={{ p: 4 }}>
              ...
              {!Boolean(state?.dataRow) && (
                <Box sx={{ marginY: "6px", marginBottom: "10px" }}>
                  ...
                  <PasswordField
                    ...
                    sx={{
                      ".MuiInputBase-root input": { minWidth: "95% !important" },
                ...
                <SoftBox sx={{ display: "flex", alignItems: "center" }}>
                  ...
                  <SoftTypography
                    ...
                    sx={{
                      fontSize: "14px",
                      color: (theme) =>
                        controls?.permissions?.includes(ele?.codename)
                          ? theme.palette.success.main
                          : theme.palette.error.main,
                      mx: 2,
                    }}
                  >
                    {controls?.permissions?.includes(ele?.codename) ? "Turned On" : "Turned Off"}
                  </SoftTypography>
                  <Switch
                    checked={controls?.permissions?.includes(ele?.codename)}
                    color="default"
                    onChange={() =>
                      setControl("permissions", [...controls?.permissions, ele?.codename])
                    }
                  />
                </SoftBox>
              </Box>
        ...
        <Stack direction="row" justifyContent="flex-end" spacing={1} className="container">
          <SoftButton
            variant="contained"
            color="white"
            onClick={() => {
          ...
          <SoftButton
            ...
            color="dark"
            sx={{
              backgroundColor: "#510ab3",
              ":hover": { backgroundColor: (theme) => theme.palette.purple.middle },
            }}
            onClick={() => handleSubmit()}
          >
            {venderResponce.isPending || vendorupdateResponce.isPending ? (
              <>
