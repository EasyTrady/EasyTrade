...
const AddBrandDialog = ({ open, handleClose }) => {
  ...
  const popularbrand = useSelector((state) => state.popularbrand.value);
    ...
    validate().then((output) => {
      ...
      if (controls.brand_type === "Popular brand") {
        RequestPostPopularBrand({
          ...
          onSuccess: (res) => {
            res?.data?.map((elem) => dispatch({ type: "brand/addItem", payload: elem }));
        ...
        {controls.brand_type === "Local brand" || "" ? (
          <>
            <Box sx={{ mt: "20px", display: "flex", alignItems: "center", gap: "20px", mb: "8px" }}>
              <img
                src={avatarUrl ? avatarUrl : imgframe}
                alt="frame"
          ...
          controls.brand_type === "Popular brand" && (
            <>
              <MultiSelect
                ...
                renderValue={(selected) => {
                  let selectedbrand = popularbrand.results?.filter((brand) =>
              ...
              <Box sx={{ mt: 2 }}>
                <Grid item>
                  {popularbrand?.results
                    ?.filter((brand) => controls.popular_brands.includes(brand?.id))
                    .map((brand) => (
                      <Grid key={brand?.id} container>
                        <Grid md={3} lg={3} pl={1} sx={{ mx: 1 }}>
                          <img
                            src={brand?.image}
                            alt="product"
                            style={{ width: "102px", height: "64px", borderRadius: "4px" }}
                          />
                        </Grid>
                        ...
                        <Grid md={1} lg={1}>
                          <Box
                            ...
                            onClick={() =>
                              setControl(
                                "popular_brands",
                                controls?.popular_brands?.filter((ele) => ele != brand?.id)
                              )
                            }
                          >
