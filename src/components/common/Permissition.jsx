...
function Permissition({children,permission,type}) {
  ...
  return (
    (type=="collapse"? permission?.some((ele)=>permissionYour.map((elem)=>elem?.codename).includes(ele)):permission?.map((ele)=>permissionYour.map((elem)=>elem?.codename).includes(ele))) &&children
  )
}
